...
All data lives under `~/.claude/forge-data/`; nothing is written into the repo.
...
- **Global lesson** (`global-lessons.jsonl`, also rendered to `~/.claude/forge-data/LESSONS.md`) — an **anonymized, transferable** behavioral correction that should make you better in *every* project. Loads into every session everywhere.
...
**1. Gather evidence.**
```
node "$HOME/.claude/skills/forge/scripts/forge-store.js" observations 60
```
...
**6. Confirm and tidy:**
```
node "$HOME/.claude/skills/forge/scripts/forge-store.js" list          # this project's instincts
node "$HOME/.claude/skills/forge/scripts/forge-store.js" list-global   # global lessons (+ path to LESSONS.md)
node "$HOME/.claude/skills/forge/scripts/forge-store.js" clear-observations
```
...
- **Prune** stale ones: `forge-store.js remove "<substring>"`. Curate ruthlessly; bloat here costs context on every session start.

## Tunables (env vars — set under `"env"` in `~/.claude/settings.json`)
**Injection (recall):**
- `FORGE_INSTINCT_MAX` / `FORGE_INSTINCT_MIN_CONF` — cap & confidence floor for **project instincts** (defaults `12` / `0`).
- `FORGE_GLOBAL_MAX` / `FORGE_GLOBAL_MIN_CONF` — cap & confidence floor for **global lessons** (defaults `15` / `0`).

**Auto-learn (the automatic distiller):**
- `FORGE_AUTOLEARN` — set to `off` to disable automatic learning entirely (default `on`).
- `FORGE_LEARN_EVERY` — distill after this many new logged actions (default `8`). Lower = more continuous + more cost; higher = leaner.
- `FORGE_LEARN_MIN_CONF` — the distiller's quality floor; lessons below this are dropped (default `0.6`).
- `FORGE_LEARN_MODEL` — model id for distillation (default Haiku).
- `FORGE_DATA_DIR` — relocate the data dir (default `~/.claude/forge-data`).

Global lessons load into every session, so keep them few and high-signal — raise `FORGE_GLOBAL_MIN_CONF` to `0.7+` once the list grows. The distiller never blocks you and silently no-ops on any error.
